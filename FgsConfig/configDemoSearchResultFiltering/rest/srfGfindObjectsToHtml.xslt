...
-->
	...
	<xsl:template name="opSpecifics">
		...
		<xsl:variable name="HITSDENIED" select="gfindObjects/@hitsDenied"/>
		...
		<h2>gfindObjects (srfType=<xsl:value-of select="$SRFTYPE"/>)</h2>
						...
						<td>
							...
							<xsl:text> </xsl:text>restXslt: 
								<select name="restXslt">
									<option value="srfGfindObjectsToHtml">srfGfindObjectsToHtml</option>
	 			...
	 			<table border="3" cellpadding="5" cellspacing="0">
					<tr>
						<td>This is hit page number <b><xsl:value-of select="$HITPAGENO"/></b>
			                ...
			                <xsl:if test="$HITSDENIED">
		 					    Search result filtering: <xsl:value-of select="$HITSDENIED"/> hits denied.
	 		                </xsl:if>
						</td>
					</tr>
				</table>
	...
	<xsl:template match="object[not(@hitDeniedNo)]">
		<tr><td>
