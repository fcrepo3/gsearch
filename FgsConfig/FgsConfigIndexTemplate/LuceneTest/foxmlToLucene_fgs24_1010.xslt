...
-->
	...
	<xsl:template match="/foxml:digitalObject" mode="activeFedoraObject">
			...
			<!-- testing of Tika extraction.
				...
				- selectedFieldnames	comma-separated list of metadata field names, if empty then all fields are included
			-->
				<IndexField IFname="ds.testMwordX" index="TOKENIZED" store="YES" termVector="NO">
				...
				<IndexField IFname="ds.testMword" index="TOKENIZED" store="YES" termVector="NO">
					<xsl:value-of disable-output-escaping="yes" select="exts:getDatastreamFromTikaWithMetadata($PID, $REPOSITORYNAME, 'testMword', 'Lucene', 'ds.', 'title,Author,Word-Count', $FEDORASOAP, $FEDORAUSER, $FEDORAPASS, $TRUSTSTOREPATH, $TRUSTSTOREPASS)"/>
				</IndexField>
				<IndexField IFname="ds.testMpdf" index="TOKENIZED" store="YES" termVector="NO">
					<xsl:value-of disable-output-escaping="yes" select="exts:getDatastreamFromTikaWithMetadata($PID, $REPOSITORYNAME, 'testMpdf', 'Lucene', 'ds.', '', $FEDORASOAP, $FEDORAUSER, $FEDORAPASS, $TRUSTSTOREPATH, $TRUSTSTOREPASS)"/>
				</IndexField>
	...
	<xsl:template name="example-of-xml-not-inline">
		...
		<!-- using the test object test:fgs24_1010 -->
		<!-- namespaces to be included in the stylesheet element -->
			
		<xsl:variable name="testMapplXml" select="document('http://localhost:8080/fedora/objects/test:fgs24_1010/datastreams/testMapplXml/content')"/>
