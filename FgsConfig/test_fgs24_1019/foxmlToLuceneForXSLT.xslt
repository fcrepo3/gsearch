...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="PID"><xsl:value-of select="/foxml:digitalObject/@PID"/>$XSLT</xsl:variable>
	...
	<xsl:template match="/">
		...
		<xsl:if test="foxml:digitalObject/foxml:objectProperties/foxml:property[@NAME='info:fedora/fedora-system:def/model#state' and @VALUE='Active']">
			<xsl:if test="not(foxml:digitalObject/foxml:datastream[@ID='METHODMAP'] or foxml:digitalObject/foxml:datastream[@ID='DS-COMPOSITE-MODEL'])">
				<xsl:if test="foxml:digitalObject/foxml:datastream[@ID='XSLT']">
	...
	<xsl:template match="/foxml:digitalObject" mode="activeFedoraObject">
			...
			<!-- indexing XSLT datastreams, text and metadata extraction using Apache Tika. -->
			
			<xsl:value-of disable-output-escaping="yes" select="exts:getDatastreamFromTika($PID, $REPOSITORYNAME, 'XSLT', 'IndexField', concat('ds.', 'XSLT'), concat('dsmd_', 'XSLT', '.'), '', $FEDORASOAP, $FEDORAUSER, $FEDORAPASS, $TRUSTSTOREPATH, $TRUSTSTOREPASS)"/>
